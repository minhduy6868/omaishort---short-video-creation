...
- Per scene: if a public start-frame URL exists and I2V returns MP4, one clip covers the scene; else zoompan per shot. Last frame = next scene still when on-camera ids overlap (HTTP Frames). Concat in scene order; no xfade (VO clock).
